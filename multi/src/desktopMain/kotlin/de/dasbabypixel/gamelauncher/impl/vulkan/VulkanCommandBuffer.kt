...
class VulkanCommandBuffer(
    ...
    val imageViews: VulkanImageViews,
    val commandPool: VulkanCommandPool
    ...
    fun recordCommandBuffer(imageIndex: Int) {
        MemoryStack.stackPush().use { stack ->
            ...
            val renderingInfo = VkRenderingInfo.calloc(stack).`sType$Default`().renderArea { area ->
                ...
                area.extent { it.set(swapChain.extent.x, swapChain.extent.y) }
            ...
            VK10.vkCmdSetScissor(commandBuffer.handle,
                ...
                VkRect2D.calloc(1, stack)
                    .offset { it.set(0, 0) }
                    .extent { it.set(swapChain.extent.x, swapChain.extent.y) })
    ...
    companion object {
        fun create(
            tracker: ResourceTracker, commandPool: VulkanCommandPool, imageViews: VulkanImageViews
        ): VulkanCommandBuffer {
            return MemoryStack.stackPush().use { stack ->
                val commandBuffer = VKCommandBuffer.create(tracker, commandPool.commandPool)
                VulkanCommandBuffer(tracker,
                    commandBuffer,
