...
class VKImageView(tracker: ResourceTracker, val handle: VkImageView, val device: VKDevice) :
    ...
    companion object {
        fun create(
            tracker: ResourceTracker, surfaceFormat: VKSurfaceFormatKHR, device: VKDevice, image: VkImage
        ): VKImageView {
            return MemoryStack.stackPush().use { stack ->
                val createInfo = VkImageViewCreateInfo.calloc(stack)
                    ...
                    .subresourceRange(VkImageSubresourceRange.calloc().set(VK10.VK_IMAGE_ASPECT_COLOR_BIT, 0, 1, 0, 1))
                ...
                VK10.vkCreateImageView(device.device, createInfo, device.pAllocator, pView).vkValidate()
