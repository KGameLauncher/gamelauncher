...
actual operator fun PlatformLoggingInstance.Companion.invoke(
    ...
    override fun log(msg: String) {
        val logger = logger as SLF4JLogger
        val location = findCaller()
        val marker = if (logger.l is MarkerLogger) MarkerManager.getMarker(logger.l.marker.name) else null
        this.logger.atLevel(Log4jLevels.PRINT_STREAM).withMarker(marker).withLocation(location).log(msg)
    }
