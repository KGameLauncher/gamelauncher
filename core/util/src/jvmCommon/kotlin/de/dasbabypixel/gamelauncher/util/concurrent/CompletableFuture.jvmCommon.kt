...
actual class CompletableFuture<T> : JCF<T> {
    ...
    override fun thenAccept(action: Consumer<in T>): CompletableFuture<Void?> =
        super.thenAccept(action).cast

    override fun thenAcceptAsync(action: Consumer<in T>): CompletableFuture<Void?> =
        super.thenAcceptAsync(action).cast
    ...
    override fun thenRunAsync(action: Runnable): CompletableFuture<Void?> =
        super.thenRunAsync(action).cast
    ...
    override fun completeAsync(supplier: Supplier<out T?>): CompletableFuture<T?> =
        super.completeAsync(supplier).cast
    ...
    override fun minimalCompletionStage(): CompletionStage<T?> =
        TODO("Minimal is not supported at this point")
    ...
    actual inline fun <V> thenApply(noinline map: (T) -> V): CompletableFuture<V> =
        ths.thenApply(map).cast

    actual inline fun <V> thenApply(map: GameFunction<in T, out V>): CompletableFuture<V> =
        ths.thenApply(map).cast
    ...
    actual companion object {
        ...
        actual fun allOf(vararg futures: CompletableFuture<*>): CompletableFuture<Unit> {
            return CompletableFuture<Unit>().also {
                JCF.allOf(*futures.toList().toTypedArray())
