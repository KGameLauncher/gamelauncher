...
class StateSynchronizer(
    private val lock: Lock = ReentrantLock(), private val condition: Condition = lock.newCondition()
) {
    private var state = 0L
    private var sleep: GameBiConsumer<Condition, Long> = GameBiConsumer { condition, nanos ->
        if (nanos < 0) {
